...
void conversieLaVector(Nod* cap, int** vectorVarste, int* dimensiuneVector) {
    while (cap) {
        (*vectorVarste)[*dimensiuneVector] = cap->info.varsta;
        (*dimensiuneVector)++;
        cap = cap->next;
    }
}
...
void main() {
    ...
    //conversie de la lista de Angajati la vector de int(vector de varste)
    int dimensiuneVector = 0;
    int* vectorVarste = (int*)malloc(6 * sizeof(int));
    conversieLaVector(cap, &vectorVarste, &dimensiuneVector);
    for (int i = 0; i < dimensiuneVector; i++) {
        printf("\nVector[%d] = %d", i, vectorVarste[i]);
    }
    free(vectorVarste);
